...
public class SetupController {
    ...
    public void processButtonClick(int rows, int cols, int players, int numWin) {
        String errorMsg = "";
        if (rows < MIN_SIZE || rows > MAX_SIZE) {errorMsg += "Rows must be between " + MIN_SIZE + " and " + MAX_SIZE;}
        if (cols < MIN_SIZE || cols > MAX_SIZE) {errorMsg += "Columns must be between " + MIN_SIZE + " and " + MAX_SIZE;}
        if (numWin > rows) {errorMsg += "Can't have more to win than the number of rows";}
        if (numWin > cols) { errorMsg += "Can't have more to win than the number of columns"; }
        if (numWin < MIN_TO_WIN) {errorMsg += "Number to win must be at least " + MIN_TO_WIN;}
